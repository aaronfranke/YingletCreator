...
namespace Character.Compositor
{
	internal static class MaterialTexturerExtensionMethods
	{
		...
		public static void ApplyTexture(this Material material, RenderTexture renderTexture, TargetMaterialTexture materialTexture)
		{
			if (materialTexture == TargetMaterialTexture.MainTexture)
				material.mainTexture = renderTexture;
			else if (materialTexture == TargetMaterialTexture.Outline)
				material.SetTexture(OUTLINE_PROPERTY_ID, renderTexture);
			else if (materialTexture == TargetMaterialTexture.Pupil)
				material.SetTexture(PUPIL_PROPERTY_ID, renderTexture);
			else if (materialTexture == TargetMaterialTexture.Mouth)
				material.SetTexture(MOUTH_PROPERTY_ID, renderTexture);
			else if (materialTexture == TargetMaterialTexture.MouthMask)
				material.SetTexture(MOUTH_MASK_PROPERTY_ID, renderTexture);
		}
	}
}
